...
function renderRepositoryContents(contents) {
  ...
  for (const item of contents) {
    if (item.type === "dir" && !excludedFolders.includes(item.name)) {
      ...
      const folderIconPath = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "path"
      );
      ...
      if (lastVisitDate) {
        ...
        lastVisitCell.textContent =
          "Terakhir dikunjungi pada: " + formattedDate;
